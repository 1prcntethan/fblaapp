    ...
    <View style={styles.hero}>
      ...
      <View style={styles.memberIdChip}>
        ...
        <Text style={styles.memberIdText}>
          Member ID: {MOCK_MEMBER.id.toUpperCase()} · Chapter {MOCK_MEMBER.chapterId}
        </Text>
      </View>
      ...
      <View style={styles.heroMetaRow}>
        ...
        <HeroMetaChip icon="time-outline"     label={`${years} yr${years !== 1 ? "s" : ""} active`} />
      </View>
    </View>
  );
}
...
function StatsBar() {
  ...
  const totalPlacements = MOCK_COMPETITIVE_EVENTS.filter((e) => e.placement !== undefined).length;
  const years           = yearsActive(MOCK_MEMBER.memberSince);
    ...
    <View style={styles.eventCard}>
      ...
      >
        {placed ? (
          <>
            ...
            <Text style={[styles.placementPlace, { color: levelMeta.color }]}>Place</Text>
          </>
        ) : (
      ...
      <View style={styles.eventCardInfo}>
        ...
        <View style={styles.eventCardMeta}>
          <View style={[styles.levelBadge, { backgroundColor: levelMeta.color + "22" }]}>
...
function CompetitiveEventsSection() {
  ...
  const displayed = activeTab === "current"
  ...
  return (
    <View style={[styles.badgeItem, badge.isLocked && styles.badgeItemLocked]}>
      ...
      <Text
        style={[
          styles.badgeTitle,
          badge.isLocked && styles.badgeTitleLocked,
        ]}
        numberOfLines={2}
      >
        {badge.title}
  ...
  return (
    <View style={styles.section}>
      ...
      <View style={styles.card}>
        ...
        {/* Detail rows */}
        <ChapterInfoRow icon="person-outline"   label="Adviser"   value={MOCK_CHAPTER_INFO.adviser} />
        <ChapterInfoRow icon="location-outline" label="Location"
          value={`${MOCK_CHAPTER_INFO.city}, ${MOCK_CHAPTER_INFO.state}`} />
        <ChapterInfoRow icon="flag-outline"     label="Region"    value={MOCK_CHAPTER_INFO.region} />
        <ChapterInfoRow icon="people-outline"   label="Members"
          value={`${MOCK_CHAPTER_INFO.totalMembers} active members`} />
        <ChapterInfoRow icon="calendar-outline" label="Founded"   value={MOCK_CHAPTER_INFO.foundedYear} />
...
function ChapterInfoRow({
  ...
  return (
    <View style={styles.chapterInfoRow}>
      ...
      <Text style={styles.chapterInfoValue} numberOfLines={1}>{value}</Text>
    </View>
  );
}
...
function SettingsSectionBlock({
  section, toggleValues, onToggle, onAction,
...
function SettingsRowItem({
  row, toggleValue, onToggle, onAction, isLast,
}: {
  row:          SettingsRow;
  toggleValue:  boolean;
  ...
  return (
    ...
    >
      {/* Icon cell */}
      <View style={[styles.settingsIcon, { backgroundColor: row.iconColor + "22" }]}>
...
export default function ProfileScreen() {
  ...
  const [toggleValues, setToggleValues] = useState<Record<string, boolean>>(initialToggles);
        ...
        Alert.alert(
          ...
          [
            { text: "Cancel", style: "cancel" },
            {
              ...
              style: "destructive",
              onPress: () => {
                // TODO: clear auth tokens, navigate to login screen
                // e.g. AuthContext.signOut(); router.replace("/login");
    ...
    <SafeAreaView style={styles.safeArea} edges={["bottom"]}>
      ...
      >
        ...
        <View style={styles.versionFooter}>
          <Text style={styles.versionText}>FBLA Member App · Version 1.0.0</Text>
